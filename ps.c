...
main(void)
{
  ...
  for (int i = 0; i < total; ++i){
        ...
        elapsed_ticks_ms = r[i].elapsed_ticks%1000;//TODO pad ms numbers
        total_ticks_secs = r[i].CPU_total_ticks/1000;
        total_ticks_ms = r[i].CPU_total_ticks%1000;//TODO pad ms
        printf(1, "%d\t %d\t %d\t %d\t %d.%d\t %d.%d\t %s\t %d\t %s\n",r[i].pid, r[i].uid, r[i].gid, r[i].ppid, elapsed_ticks_secs, elapsed_ticks_ms, total_ticks_secs, total_ticks_ms, r[i].state, r[i].size, r[i].name);
    } 
