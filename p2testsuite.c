...
//Test by Jordan Green, I made some time/size changes and the zombies are no longer killed in the test.
  static void
test_round_robin()
{
  //int mistakes[64];
...
//Test by Jordan Green .. I've made changes to the times but otherwise have not changed the test.
static void
test_kill_reap(){
 ...
 */
//TODO This is the start of the original, the tests above are the P3 ones
...
#endif
#ifdef GETPROCS_TEST
  testgetprocs();  // no need to pass argv[0]//TODO comment back in when working
#endif
#ifdef TIME_TEST
  testtime();
#endif
